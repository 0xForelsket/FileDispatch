...
function App() {
  ...
  return (
    ...
    >
      {/* Header */}
      <header className={`h-12 bg-[var(--bg-header)] border-b flex items-center justify-between px-4 shrink-0 relative z-10 transition-all ${
        isLinear ? "border-[var(--border-main)] shadow-sm" : "border-[var(--border-main)] shadow-[0_0_15px_var(--border-main)] border-b-4"
      }`}>
      ...
      <div className={`flex-1 flex min-h-0 bg-[var(--bg-app)]/80 ${
        isLinear ? "gap-3 p-3" : "p-4 gap-4"
      }`}>
        
        {/* Pane 1: Folders */}
        <aside className={`w-[260px] flex flex-col relative transition-all duration-300 ${
          isLinear ? "bg-[var(--bg-panel)] border border-[var(--border-main)]" : "magi-border bg-black"
        }`}>
           {!isLinear && <AbsoluteCornerDecorations />}
           <div className={`flex justify-between items-center select-none shrink-0 mb-1 ${
               isLinear 
        ...
        {/* Pane 2: Rules */}
        <section className={`w-[340px] flex flex-col relative transition-all duration-300 ${
             isLinear ? "bg-[var(--bg-panel)] border border-[var(--border-main)]" : "magi-border bg-[var(--bg-panel)]"
        }`}>
          ...
          <div className={`flex justify-between items-center select-none shrink-0 mb-1 ${
            ...
            <button
               ...
               className={`text-xs font-semibold rounded-[var(--radius)] px-2 py-0.5 transition-colors ${
                 isLinear
        ...
        {/* Pane 3: Editor */}
        <section className={`flex-1 flex flex-col min-w-0 relative transition-all duration-300 ${
            isLinear ? "bg-[var(--bg-panel)] border border-[var(--border-main)]" : "magi-border bg-[var(--bg-panel)]"
        }`}>
